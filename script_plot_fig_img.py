...
import plotting
from gen_data import generate_image_recog_data
...
N = d = 25
for img_type in ["OBJECTSALL", "STATE"]:
    ax = plotting.plot_loss_acc('HebbNet[{},{},1]_forceAnti_train=dat3_T=5000_{}.pkl'.format(d,N,img_type), chance=2./3)
    format_and_save(ax[0].get_figure(), f'{img_type}_train_val/AntiHebb_d={d}_N={N}_R=3_iters=4000.pdf', w=83, h=100)
...
for img_type in ["OBJECTSALL", "STATE"]:
    for force in ['Anti']:
        ax = None
        for R in [3,6]:
            fname = 'HebbNet[{},{},1]_force{}_train=inf{}_T={}_{}.pkl'.format(d,N,force,R, 5000,img_type)
        ...
        format_and_save(ax[0].get_figure(), './{}_plots/accuracy_[{},{},1]_force={}.pdf'.format(img_type, d, N, force), w=58, h=70)
...
for img_type in ["OBJECTSALL", "STATE"]:
    ...
    data1 = generate_image_recog_data(img_type, T=5000, R=1, d=d, P=0.5, multiRep=False)
    for i,Rtrain in enumerate([1,7,14]):
        fname = 'HebbNet[{},{},1]_force{}_train=inf{}_T={}_{}.pkl'.format(d,N,force,Rtrain, 5000, img_type)
        net = load_from_file(fname)
        ...
        data = generate_image_recog_data(img_type,T=5000, R=Rtrain, d=d, P=0.5, multiRep=False)
    ...
    format_and_save(ax[0][0].get_figure(), f'mechanism_187_{img_type}/mechanism_Rtrain={[1,7,14]}_d={d}_N={N}_h.pdf', w=200, h=200)
    format_and_save(ax[1][0].get_figure(), f'mechanism_187_{img_type}/mechanism_Rtrain={[1,7,14]}_d={d}_N={N}_W1.pdf', w=45, h=100)
    format_and_save(ax[2][0].get_figure(), f'mechanism_187_{img_type}/mechanism_Rtrain={[1,7,14]}_d={d}_N={N}_Wxb.pdf', w=58, h=100)
