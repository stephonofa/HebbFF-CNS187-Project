...
for d in range(25,151,25):
    N = d
    ax = plotting.plot_loss_acc('HebbNet[{},{},1]_forceAnti_train=dat3_T=5000_(2).pkl'.format(d,N), chance=2./3)
    format_and_save(ax[0].get_figure(), f'train_val/AntiHebb_d={d}_N={N}_R=3_iters=4000.pdf', w=83, h=100)
...
for d in range(25,151,25):
    ...
    for force in ['Anti']:
        ax = None
        for R in [3,6]:
            fname = 'HebbNet[{},{},1]_force{}_train=inf{}_T={}.pkl'.format(d,N,force,R, 5000)
        ...
        format_and_save(ax[0].get_figure(), './accuracyplots/accuracy_[{},{},1]_force={}.pdf'.format(d, N, force), w=58, h=70)
...
for d in range(25,151,25):
    ...
    data1 = generate_recog_data(T=5000, R=1, d=d, P=0.5, multiRep=False)
    for i,Rtrain in enumerate([1,7,14]):
        fname = 'HebbNet[{},{},1]_force{}_train=inf{}_T={}.pkl'.format(d,N,force,Rtrain, 5000)
        net = load_from_file(fname)
        ...
        data = generate_recog_data(T=5000, R=Rtrain, d=d, P=0.5, multiRep=False)
    ...
    format_and_save(ax[0][0].get_figure(), f'mechanism_187/mechanism_Rtrain={[1,7,14]}_d={d}_N={N}_h.pdf', w=200, h=200)
    format_and_save(ax[1][0].get_figure(), f'mechanism_187/mechanism_Rtrain={[1,7,14]}_d={d}_N={N}_W1.pdf', w=45, h=100)
    format_and_save(ax[2][0].get_figure(), f'mechanism_187/mechanism_Rtrain={[1,7,14]}_d={d}_N={N}_Wxb.pdf', w=58, h=100)
