...
from gen_data import generate_mnist_recog_data, generate_mnist_recog_data_batch
...
#train
if trainMode == 'dat':
    trainData = generate_mnist_recog_data_batch(T=T, d=d, R=R, P=0.5, multiRep=False)
    validBatch = generate_mnist_recog_data(T=T, d=d, R=R, P=0.5, multiRep=False).tensors
    net.fit('dataset', epochs=4000, trainData=trainData,
            validBatch=validBatch, earlyStop=False)
elif trainMode == 'inf':
    gen_data = lambda: generate_mnist_recog_data_batch(T=T, d=d, R=R, P=0.5, multiRep=False)
...
#optional save
if save:
    fname = '{}[{},{},1]_{}train={}{}_{}_mnist.pkl'.format(
